    ...
    if (this.input instanceof tf.Tensor) {
      ...
      switch (this.kind) {
        case "image":
          ...
          break;
        case "video":
          this.tensor = await this.env.decodeVideoToTensors(this.input, options);
          break;
        case "tiff":
  ...
  async toBlob() {
    ...
    if (this.kind === "tensor") {
      if (typeof this.env.tensorToBlob === "function") {
        return await this.env.tensorToBlob(this.tensor, this);
      }
    ...
    if (this.structure === "complex") {
      if (typeof this.env.complexToBlob === "function") {
        return await this.env.complexToBlob(this.tensor, this);
      }
